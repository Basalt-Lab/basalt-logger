...
/**
 * Tests for the BasaltLogger class (immutable API).
 */
describe('BasaltLogger', () => {
    ...
    describe('unregisterStrategies', () => {
        test('should remove multiple strategies and return a new BasaltLogger instance', () => {
            ...
            const logger3 = (logger2 as unknown as { unregisterStrategies: (names: string[]) => BasaltLogger }).unregisterStrategies(['test1', 'test2']);
        ...
        test('should throw an error if a strategy is not found', () => {
            ...
            const logger2 = basaltLogger.registerStrategies(strategies);
            expect(() => (logger2 as unknown as { unregisterStrategies: (names: string[]) => BasaltLogger }).unregisterStrategies(['test1', 'test3'])).toThrow('The strategy "test3" is not found.');
        });
    });
    ...
    describe('logging', () => {
        test.each([
            ...
            const logger = new BasaltLogger().registerStrategy('test', strategy);
    ...
    describe('internal behavior', () => {
        test('should ignore logs when maxPendingLogs is reached', () => {
            ...
            for (let i = 0; i < 10; i++)
                logger.log(`log${i}`);
            const loggerFull = new BasaltLogger({}, 1).registerStrategy('test', strategy);
        ...
        test('should handle backpressure in _writeLog', async () => {
            ...
            const logger = new BasaltLogger().registerStrategy('test', mockStrategy);
            logger.log('test1');
            logger.log('test2');
